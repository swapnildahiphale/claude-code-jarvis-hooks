...
def prompt_llm(prompt_text):
    ...
    if not api_key:
        debug_print("Missing API key. Expected env var: ANTHROPIC_API_KEY")
        return None
    
    debug_print(f"API Key found: {api_key[:10]}...")
    debug_print(f"Model: claude-3-5-haiku-20241022")
...
def generate_notification_message():
    """
    Generate a notification message using Anthropic LLM.
